...
| **Multi-task 신경망** | ResNet18을 특징 추출기(Backbone)로 사용하고, 위치 예측(Regression)과 종류 판별(Classification)을 위한 각각의 Head 설계. | 단순히 두 Loss를 더하는 것을 넘어, DetectionLoss 클래스에 alpha, beta 가중치 파라미터를 도입하여 스케일이 다른 위치 오차(MSE)와 분류 오차(CE) 간의 학습 불균형을 제어하는 실무적 밸런싱 감각을 익힘. |
